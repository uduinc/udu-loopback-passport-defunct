...
Strats.init = function ( app )
{
    Strats.app = app;
}

Strats.loginOrRegisterUser = function ( accessToken, profile, done )
{
    ...
    var gender = profile.gender; // Atm only gatehred from Facebook; investigate Google/Twitter.
    ...
    // memberModel.findById( id, function ( fErr, fData )
    // {
    //     var contacts = _.pluck( fData.contacts, 'contact' );
    //     console.log( 'contacts = ', contacts );

    //     if ( contacts.indexOf( contact ) !== -1 )
    //     {
    //         cb( null, 'Contact: ' + contact + ' is already a part of this user.' );
    //     }
    //     else
    //     {
    //         console.log( 'Going to mongo...' );
    //         var updateObj = { contacts: fData.contacts };
    //         updateObj.contacts.push(
    //             {
    //                 contact: contact,
    //                 contactType: Member.utils.detectContactType( contact ),
    //                 notify:     true
    //             }
    //         );      
    //         // It doesn't have a single-update method that'll work for us,
    //         // so we just use the user's unique _id to render its 'multi' option impotent
    //         Member.updateAll( searchObj, updateObj, function ( uErr, uData )
    //         {
    //             console.log( 'uErr = ', uErr );
    //             console.log( 'uData = ', uData + ' (# of documents updated -- should be 1)' );
    //             cb( null, contact );
    //         });
    //     }
    // });      
...
Strats.facebook = function ( accessToken, refreshToken, profile, done )
{
    ...
    {
        Strats.loginOrRegisterUser( accessToken, profile, done );        
    }    
};
